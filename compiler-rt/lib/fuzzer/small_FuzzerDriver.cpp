...
// Copied from AFL
int BindToFreeCPU(int SleepTimeUsec) {
  // find runnable cpu cores number
  uint32_t CPUCoreCount = 0, i;
  ...
  while (fgets(tmp, sizeof(tmp), f))
    if (!strncmp(tmp, "cpu", 3) && isdigit(tmp[3])) CPUCoreCount++;
  fclose(f);
  ...
  uint8_t CPUUsedSet[4096] = {0};
  const uint32_t MAX_LINE = 0x1000;
  char TmpContent[MAX_LINE];
  cpu_set_t c;
  // sleep in case multiple libfuzzer doing the same thing
  usleep(SleepTimeUsec * 1000);
  ...
  while ((DirectoryEntry = readdir(Directory))) {
    char CurrentFileName[0x1000];
    FILE *CurrentFile;
    bool HasVMSize = false;

    if (!isdigit(DirectoryEntry->d_name[0])) continue;

    snprintf(CurrentFileName, 0xfff, "/proc/%s/status", DirectoryEntry->d_name);
    if (!(CurrentFile = fopen(CurrentFileName, "r"))) {
      fprintf(stderr, "open %s failed\n", CurrentFileName);
      continue;
    }

    while (fgets(TmpContent, MAX_LINE, CurrentFile)) {
      uint32_t NowCore;

      if (!strncmp(TmpContent, "VmSize:\t", 8)) HasVMSize = true;

      if (!strncmp(TmpContent, "Cpus_allowed_list:\t", 19) &&
          !strchr(TmpContent, '-') && !strchr(TmpContent, ',') &&
          sscanf(TmpContent + 19, "%u", &NowCore) == 1 && NowCore < sizeof(CPUUsedSet) &&
          HasVMSize) {
        CPUUsedSet[NowCore] = 1;
        break;  
      }
    }
    fclose(CurrentFile);
  ...
  closedir(Directory);
  for (i = 0; i < CPUCoreCount; ++i) if (!CPUUsedSet[i]) break;

  if (i == CPUCoreCount) {
    fprintf(stderr, "no free cpu core is available!\n");
    exit(255);
  }
...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
  ...
  Options.UnitTimeoutMSec = Flags.timeout_msec;
  ...
  if (Flags.file_name)
    Options.FileNameLength = snprintf(Options.FileName, 0xff, "%s", Flags.file_name);
  Options.ForkServer = Flags.fork_server;
  ...
  // Bind to a free cpu
  int CpuNumber = BindToFreeCPU(Rand(1000));
  // use the cpu core number it bind to
  Options.FuzzingNumber = CpuNumber;
