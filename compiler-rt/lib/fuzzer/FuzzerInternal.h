...
public:
  ...
  void InstallInterruptCallback(int signum);
...
private:
  system_clock::time_point StartFuzzingTime;

  bool Timeout = false;
  bool UseFile = false;
  int ForkServerPid;
  int FuzzingFileFD = 0;
  int FuzzingStatusFileFD = 0;
  int FuzzingStdioFileFD = 0;
  char FuzzingFileName[0x100];
  char FuzzingStatusFileName[0x100];
  char *FuzzingFile;
  int ReadStartFromParentFD, WriteStartToForkServerFD;
  int ReadStatusFromForkServerFD, WriteStatusToParentFD;
